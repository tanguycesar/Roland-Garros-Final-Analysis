...
def make_frame_features(kin: Dict[str, np.ndarray], cfg: FeatureConfig) -> Tuple[np.ndarray, List[str]]:
    """Crée features basées sur physique du court avec contexte étendu"""
    ...
    w = 5  # Fenêtre : 11 frames (5 avant, actuelle, 5 après)
...
def visualize_dashboard(frames, kin, probs, final_actions):
    ...
    axes[0].axhline(11.88, color='red', ls=':', alpha=0.5)
    axes[0].axhline(-11.88, color='red', ls=':', alpha=0.5)
    ...
    axes[0].invert_yaxis()
    axes[0].set_title("Trajectoire & Détections")
