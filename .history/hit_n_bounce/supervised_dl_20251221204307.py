...
class EventPostProcessor:
    ...
    - Non-Maximum Suppression (NMS) temporelle
    - Filtrage par distance minimale entre événements
    """
    ...
    def __init__(
        ...
        """
        Args:
            confidence_threshold: Seuil de probabilité pour considérer un événement
            nms_window: Taille de fenêtre pour NMS (frames)
            min_event_distance: Distance minimale entre deux événements (frames)
        """
    ...
    def apply_nms(
        ...
        class_id: int = 1  # 1=hit, 2=bounce
    ) -> List[int]:
        """
        Applique Non-Maximum Suppression pour extraire les pics.
        
        Returns:
            Liste des indices de frames détectées
        """
        # Probabilités pour la classe d'intérêt
        class_probs = probabilities[:, class_id]
        
        # Recherche de pics avec scipy
    ...
    def extract_events(
        ...
        for peak in hit_peaks:
            confidence = probabilities[peak, 1]
            if confidence >= threshold_hit:
                results['hits'].append((peak, confidence))
        
        # Bounces (classe 2)
        bounce_peaks = self.apply_nms(probabilities, class_id=2)
        for peak in bounce_peaks:
            confidence = probabilities[peak, 2]
            if confidence >= threshold_bounce:
                results['bounces'].append((peak, confidence))
    ...
    def visualize_detections(
        ...
        detections: Dict[str, List[Tuple[int, float]]],
        figsize: Tuple[int, int] = (15, 8)
    ):
        """
        Visualise les détections vs ground truth.
        """
        fig, axes = plt.subplots(3, 1, figsize=figsize, sharex=True)
        frames = np.arange(len(probabilities))
        
        # Probabilités par classe
        for class_id, (ax, label, color) in enumerate(zip(
            axes,
            ['Air', 'Hit', 'Bounce'],
            ['gray', 'red', 'blue']
        )):
            ax.plot(frames, probabilities[:, class_id], label=f'P({label})', 
                   color=color, alpha=0.6, linewidth=1.5)
            ax.axhline(self.confidence_threshold, color='black', 
                      linestyle='--', alpha=0.3, label='Threshold')
            
            # Ground truth
            gt_frames = np.where(ground_truth == class_id)[0]
            if len(gt_frames) > 0:
                ax.scatter(gt_frames, [0.9] * len(gt_frames), 
            ...
            if class_id == 1:  # Hits
                for frame, conf in detections['hits']:
                    ax.axvline(frame, color='red', alpha=0.5, linewidth=2)
                    ax.scatter(frame, conf, marker='*', color='red', 
                             s=200, zorder=6, edgecolors='black')
            elif class_id == 2:  # Bounces
                for frame, conf in detections['bounces']:
                    ax.axvline(frame, color='blue', alpha=0.5, linewidth=2)
                    ax.scatter(frame, conf, marker='*', color='blue', 
                             s=200, zorder=6, edgecolors='black')
...
class TennisEventMetrics:
    ...
    def compute_metrics(
        ...
        """
        Calcule F1, Precision, Recall, Confusion Matrix, PR-AUC.
        """
        # Métriques de base
        ...
        # PR-AUC si probabilités disponibles
        if y_proba is not None:
            ...
            for class_id, class_name in enumerate(LABELS):
                ...
                if len(np.unique(y_true_binary)) > 1:  # Au moins 1 exemple positif
        ...
        for class_id, (ax, class_name) in enumerate(zip(axes, LABELS)):
            ...
            if len(np.unique(y_true_binary)) > 1:
                ...
                ax.plot(recall, precision, linewidth=2, 
                       label=f'PR-AUC = {auc:.4f}')
                ax.fill_between(recall, precision, alpha=0.2)
            else:
                ax.text(0.5, 0.5, 'Pas d\'exemples positifs', 
                       ha='center', va='center')
...
# =========================================================================
# 6. FEATURE ENGINEERING POUR FENÊTRES
# =========================================================================
